...
public class LoadingLogin extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ConexionSQLiteHelper conn = new ConexionSQLiteHelper(this, "lifeguard", null, 2);
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            NotificationChannel channel = new NotificationChannel("Alerta LifeGuard", name, importance);
    ...
    protected void onStart() {
        ...
        (new Handler()).postDelayed(new Runnable() {
            public void run() {
                if(currentUser!=null){
                    Log.d("LOG", "Usuario logueado: "+currentUser.getEmail()+" verificado: "+currentUser.isEmailVerified());
                    if(!currentUser.isEmailVerified()){
                        ...
                        ConexionSQLiteHelper mConectionSQLiteHelper = new ConexionSQLiteHelper(getApplicationContext(), "lifeguard", null, 2);
                        SQLiteDatabase readingDatabase = mConectionSQLiteHelper.getReadableDatabase();
                        Cursor cursor = readingDatabase.rawQuery("SELECT * FROM usuario WHERE Uid LIKE '"+currentUser.getUid()+"'", null);
                        if (cursor.getCount()==0){
                            ...
                            //¿De qué sirve esto aquí? El current user no es null y ya está en la BD.
                            ...
                            userValues.put("edad", "");
                            userValues.put("mensaje", "Me encuentro en una emergencia. A continuación se muestra mi ubicación actual y algunos datos personales.");
        ...
        (new Handler()).postDelayed(new Runnable() {
            public void run() {
                if(!user.isEmailVerified()){
                    Toast.makeText(getApplicationContext(), "La dirección de correo no ha sido verificada aún, intente de nuevo.", Toast.LENGTH_LONG).show();
                }else{
                    Toast.makeText(getApplicationContext(), "La dirección ha sido verificada.", Toast.LENGTH_LONG);
    ...
    public void createAccount(View view){
        ...
        if (!emailCA.matches("") && !passwordCA.matches("") && !txt.getText().toString().matches("") && !phone.getText().toString().matches(""))
                ...
                .addOnCompleteListener(this, new OnCompleteListener<AuthResult>() {
                    ...
                    public void onComplete(@NonNull Task<AuthResult> task) {
                        if (task.isSuccessful()) {
                            ...
                            userValues.put("telefono", Long.valueOf(tel.substring(tel.length()-10)));
                            ...
                            Long idRes = writingDatabase.insert("usuario", null, userValues);
                            newUser = new UserInfo(user.getUid(), txt.getText().toString(), Long.valueOf(phone.getText().toString()), user.getEmail(),0,Long.valueOf(0), null,null, null, null, null, null);
                            Toast.makeText(getApplicationContext(), "Usuario agregado.", Toast.LENGTH_LONG).show();
                            ...
                            Toast.makeText(getApplicationContext(), "Error al crear usuario, intente de nuevo.",
                                    Toast.LENGTH_SHORT).show();
                        }
                    }
                });
        else Toast.makeText(getApplicationContext(), "Ingrese todos los datos, por favor.", Toast.LENGTH_LONG).show();
    }
                ...
                .addOnCompleteListener(this, new OnCompleteListener<AuthResult>() {
                    ...
                    public void onComplete(@NonNull Task<AuthResult> task) {
                        if (task.isSuccessful()) {
                            ...
                            Toast.makeText(getApplicationContext(), "Error al ingresar, intente de nuevo.",
